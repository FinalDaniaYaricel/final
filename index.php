...
$col_count = 0;
        ...
        $query = "SELECT a.`id`, a.`title`, a.`image` FROM `anuncio` AS a INNER JOIN `user` AS u ON a.`fb_id` = u.`fb_id` WHERE a.`state`=1 AND u.`state`=1 AND a.`title` LIKE '%{$_GET['search']}%'";
			}else{
        $query = "SELECT a.`id`, a.`title`, a.`image` FROM `anuncio` AS a INNER JOIN `user` AS u ON a.`fb_id` = u.`fb_id` WHERE a.`state`=1 AND u.`state`=1 ORDER BY a.`id` DESC LIMIT 9";
			}
